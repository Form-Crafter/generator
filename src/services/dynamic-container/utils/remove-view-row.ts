...
// реализовать isHide и проверить col: auto, кажется там пиздец при скрытии будет(
// добавить всем типам контейнера и динамк. контейнера callback на скрытие/показ компонента в рамках текущего view
// добавить в options builder прототип callback на определённые действия multifiedl (и других полей если нужно)
// добавить таже флаги isValidationsDisabple/isRelationsDisabple

type CollectIds = { rows: EntityId[]; components: EntityId[] }
